...
library(checkpoint)
checkpoint("2017-01-08")
setSnapshot("2017-01-08")
if(!require(dplyr)){
...
# Mice 3/timepoint alternative bleeding IP only

```{r DataSim_Hierarch_IP}
times <- c(5 / 60, 0.5, 1, 3, 4, 6, 8, 24)
Dosage <- 6 # mg/kg
Weights <- rnorm(9, 25, 1) / 1000 # in kg
V_mass <- 58.5 # ml of blood / kg
V_ds <- V_mass * Weights
Dose <- 1500

t_elims <- log(2) / rnorm(9, 6, 0.3)
t_dists <- log(2) / rgamma(9, shape = 4, rate = 12)
partitions <- rnorm(9, 0.6, 0.1)

t_absorp_IPs <- log(2) / abs(rnorm(9, 0.5, 0.1))

F_IPs <- replicate(9, min(c(0.99, rnorm(1, 0.9, 0.1))))

IP_inpus <- matrix(rep(0, 72), ncol = 9)
IP_dists <- matrix(rep(0, 72), ncol = 9)
IP_elims <- matrix(rep(0, 72), ncol = 9)
IP_dats <- matrix(rep(0, 72), ncol = 9)
...
indiv <- as_data_frame(IP_dats) %>%
  mutate(Time = times)

indiv_plot <- indiv %>%
  gather("Animal", "Concentration", 1:9) %>%
  mutate(Animal_ID = rep(1:9, each = 8))

ggplot(indiv_plot, aes(Time, Concentration)) +
  geom_point() +
  geom_line() +
  facet_wrap(~Animal) +
  theme_minimal()
...
final_dat <- filter(indiv_plot,
                    (Animal_ID <= 3 & Time %in% c(5/60, 3, 8, 24)) | 
                      (Animal_ID > 3 & Animal_ID <= 6 & Time %in% c(0.5, 4, 24)) |
                      (Animal_ID > 6 & Time %in% c(1, 6, 24)))
```

```{stan output.var='HalfLife_Mod_ML_IP'}
data{
int N;
int N_anim;
vector[N] times;
...
transformed data{
vector[N] conc_ip_log;

conc_ip_log = log(conc_ip);
}
parameters{
//means
real log_t_05_mu; // expressed as ln(2)/t_05 so elimination first then distribution
...
//SDs
real<lower = 0> t_05_sigma;
real<lower = 0> t_dist_sigma;
real<lower = 0> t_IP_sigma;
real<lower = 0> V_d_sigma;
//Raw values
vector[N_anim] t_elim_raw;
vector[N_anim] t_dist_raw;
vector[N_anim] t_IP_raw;
vector[N_anim] V_d_raw;

//Fractional values are beta-distributed
vector<lower = 0, upper = 1>[N_anim] partition;
vector<lower = 0, upper = 1>[N_anim] frac_IP;
//vector<lower = 0, upper = 1>[N_anim] dist_prop; // distribution:elimination half-life proportion
real<lower = 0> partition_suc;
real<lower = 0> frac_IP_suc;
//real<lower = 0> dist_prop_suc;
real<lower = 0> partition_fail;
...
transformed parameters{
vector<lower = 0>[N_anim] t_elim;
vector<lower = 0>[N_anim] t_dist;
vector<lower = 0>[N_anim] t_IP;
vector<lower = 0>[N_anim] V_d;

vector[N] calc_ip;

t_elim = exp(log_t_05_mu + t_elim_raw * t_05_sigma);
t_dist = t_elim + exp(log_t_dist_mu + t_dist_raw * t_dist_sigma);
t_IP = exp(log_t_IP_mu + t_IP_raw * t_IP_sigma);
V_d = exp(log_V_d_mu + V_d_raw * V_d_sigma);
...
for(i in 1:N){
  ...
  ip_input = ((dose * frac_IP[animal[i]]) * (1 - exp(-(times[i] * t_IP[animal[i]])))) / V_d[animal[i]];
  calc_ip[i] = log((partition[animal[i]] * ip_input) * exp(-(times[i] * t_dist[animal[i]])) +
    ((1 - partition[animal[i]]) * ip_input) * exp(-(times[i] * t_elim[animal[i]])));
}

}
model{

log_t_05_mu ~ normal(-log(6), 1);
log_t_dist_mu ~ normal(1, 2);
log_t_IP_mu ~ normal(0, 2);
log_V_d_mu ~ normal(0, 2);

//SDs
t_05_sigma ~ normal(0, 1);
t_dist_sigma ~ normal(0, 1);
t_IP_sigma ~ normal(0, 2);
V_d_sigma ~ normal(0, 5);
//Raw values
t_elim_raw ~ normal(0, 1);
...
frac_IP ~ beta(frac_IP_suc, frac_IP_fail);
//dist_prop ~ beta(dist_prop_suc, dist_prop_fail);
partition_suc ~ student_t(7, 0, 3);
frac_IP_suc ~ student_t(7, 0, 3);
//dist_prop_suc ~ student_t(7, 0, 3);
partition_fail ~ student_t(7, 0, 3);
frac_IP_fail ~ student_t(7, 0, 3);
//dist_prop_fail ~ student_t(7, 0, 3);


dispersion ~ normal(0, 1);

conc_ip_log ~ normal(calc_ip, dispersion);

#target += sum(log_t_dist_mu + t_dist_raw * t_dist_sigma);
}
generated quantities{
vector[N_anim] t_dist_lin;
vector[N_anim] t_elim_lin;
...
```{r RunMLModel_IP}
res <- sampling(HalfLife_Mod_ML_IP, data = list(N = nrow(final_dat),
                                                N_anim = 9,
                                                times = final_dat$Time,
                                                conc_ip = final_dat$Concentration,
                                                dose = 1500,
                                                animal = final_dat$Animal_ID),
                control = list(adapt_delta = 0.9999), chains = 4, iter = 2000,
                warmup = 500, refresh = 50)
saveRDS(res, "HalfLife_Mod_ML_IP.RDS")
print(res, pars = c("log_t_05_mu", "t_05_sigma",# "dist_prop",
                    "partition_suc", "partition_fail",
                    "log_V_d_mu", "V_d_sigma", "dispersion",
                    "log_t_IP_mu", "t_IP_sigma",
                    #"dist_prop_suc", "dist_prop_fail",
                    "frac_IP_suc", "frac_IP_fail",
                    "t_dist", "t_elim"))
...
for(i in 1:1000)
  for(j in 1:9){
    temp <- CalcIP(Dose, times, Vs[i, j], elims[i, j], dists[i, j],
                    ip_frac[i, j], ip_ins[i, j], parts[i, j]) %>%
      mutate(Sample = i,
             Animal = paste("V", j, sep = ""),
             Group = paste(i, "V", j, sep = ""))
    dt <- bind_rows(dt, temp)
...
ggplot(indiv_plot, aes(Time, Concentration)) +
  geom_line(data = dt, alpha = 0.01, aes(group = Group)) +
  geom_point(colour = "blue") +
  geom_point(data = final_dat, colour = "red") +
  facet_wrap(~Animal) +
  theme_minimal()
```
...
ggplot(indiv_plot, aes(Time, Concentration)) +
  geom_point(data = dt, alpha = 0.01, aes(group = Group)) +
  geom_point(colour = "blue") +
  geom_point(data = final_dat, colour = "red") +
  facet_wrap(~Animal) +
  theme_minimal()
