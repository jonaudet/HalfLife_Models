...
final_dat <- filter(indiv_plot,
                    (Animal_ID <= 3 & Time %in% c(5/60, 3, 8)) | 
                      (Animal_ID > 3 & Animal_ID <= 6 & Time %in% c(0.5, 4, 24)) |
                      (Animal_ID > 6 & Time %in% c(1, 6)))
```
...
parameters{
//means
real<lower = 0> t_05_mu; // expressed as ln(2)/t_05 so elimination first then distribution
real<lower = 0> t_dist_mu;
real log_t_IP_mu;
real log_V_d_mu;
//SDs
real<lower = 0> t_05_sigma;
...
transformed parameters{
ordered<lower = 0>[2] t_elim_dist[N_anim];
...
for(i in 1:N_anim){
  t_elim_dist[i, 1] = log(2) ./ (t_05_mu + t_elim_raw[i] * t_05_sigma);
  t_elim_dist[i, 2] = log(2) ./ (t_dist_mu + t_dist_raw[i] * t_dist_sigma);
}
t_IP = exp(log_t_IP_mu + t_IP_raw * t_IP_sigma);
V_d = exp(log_V_d_mu + V_d_raw * V_d_sigma);
...
for(i in 1:N){
  ...
  calc_ip[i] = log((partition[animal[i]] * ip_input) * exp(-(times[i] * t_elim_dist[animal[i], 2])) +
    ((1 - partition[animal[i]]) * ip_input) * exp(-(times[i] * t_elim_dist[animal[i], 1])));
}
...
partition ~ beta(partition_suc, partition_fail);
frac_IP ~ beta(frac_IP_suc, frac_IP_fail);
partition_suc ~ gamma(1.5, 6);
frac_IP_suc ~ gamma(1.5, 6);
partition_fail ~ gamma(1.5, 6);
frac_IP_fail ~ gamma(1.5, 6);
...
for(i in 1:N_anim){
  t_dist_lin[i] = log(2) / t_elim_dist[i, 2];
  t_elim_lin[i] = log(2) / t_elim_dist[i, 1];
}
}
```
...
res <- sampling(HalfLife_Mod_ML_IP, data = list(N = nrow(final_dat),
                ...
                warmup = 500, refresh = 50, diagnostic_file = "dia", verbose = T)
